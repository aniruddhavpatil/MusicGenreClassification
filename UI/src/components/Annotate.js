...
class Annotate extends Component {
  ...
  forceUpdate(done){
    ...
    // let newList = oldList.filter((val,ind,arr)=> {
    //   return arr[ind]._id.$oid != done
    // })
  ...
  populate(list){
    ...
    for(var row=0; row < this.state.rows; row++){
      ...
      for(var column = 0; column < this.state.columns; column++){
        ...
        // console.log(url)
        vidRow.push(
            <VideoPlayer reRender={(i)=>this.forceUpdate(i)} user={this.props.username} key={row * this.state.columns + column} url={url} id={vidId} fps={fps}/>
        );
      }
